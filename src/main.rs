...
async fn main() -> Result<()> {
    ...
    let stt = SttPair {
        user:   Box::new(DeepgramSender::connect(Speaker::User,   transcript_tx.clone(), &api_key).await?),
        system: Box::new(DeepgramSender::connect(Speaker::System, transcript_tx,         &api_key).await?),
    };

    audio::wasapi::start_concurrent_capture(audio_tx)?;
    tokio::spawn(pipeline::run(audio_rx, pause_flag, stt));
